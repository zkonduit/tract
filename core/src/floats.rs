...
impl<T1: Datum + Float, T2: Datum + Float> FloatPrecisionTranslator<T1, T2> {
    pub fn with_filter(node_predicate: impl Fn(&TypedNode) -> bool + 'static) -> Self {
        Self {
            node_predicate: Some(Box::new(node_predicate)),
    ...
    fn cast_inputs_if_required(
        ...
        let original_op_float_dt = if op_float_dt == T1::datum_type() {
            T2::datum_type()
        } else {
            T1::datum_type()
        };
    ...
    fn translate_node(
        ...
        if !self.should_translate_node(node) && !is_source {
            ...
            let new_op = if let Some(source) = node.op_as::<TypedSource>() {
                Box::new(TypedSource::new(fact_float_precision_conversion::<T1, T2>(
                    &source.fact,
                )))
...
mod test {
    ...
    fn build_f32_model() -> TractResult<TypedModel> {
        ...
        let add = model
            .wire_node("layer.0/add", math::add(), &[a, a])
            .unwrap()[0];
        let mul = model
            .wire_node("layer.0/mul", math::mul(), &[add, multiplier])
            .unwrap()[0];
        let pow = model
            .wire_node("layer.1/pow", math::pow(), &[mul, pow_factor])
            .unwrap()[0];
        let _output = model
            .wire_node(
                "layer.1/add_neg_infinity",
                math::add(),
                &[pow, neg_infinity],
            )
            .unwrap()[0];
